...
class Theme {
  ...
  /**
   * Gets a rendered icon component from its element
   * @param {Element} The element used to report the location of the icon from the document.
   * @returns {?Icon} Returns the icon found, ortherwise null.
   */
  getIconByElement(element) {
    if (this.container.contains(element) && icons.has(element)) {
      const icon = icons.get(element);
  ...
  /**
   * Finds one icon in using its name
   * @param {String} The name of the icon.
   * @returns {?Icon} Returns the icon found, ortherwise null.
   */
  findOne(name) {
    const element = $(`[data-icon='${name}']`, this.container);
    return (element) ?
      this.getIconByElement(element) :
      null;
  }

  /**
   * Finds many icons in using its names
   * @param {String} The name of the icons.
   * @returns {Array:Icon} Returns the icons found.
   */
  findMany(name) {
    let icons = [];
    $$(`[data-icon='${name}']`, this.container).forEach(function(elem) {
      icons.push(this.getIconByElement(elem));
    }, this);
  }
